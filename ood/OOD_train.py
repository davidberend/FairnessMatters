...
def Convert(train_path, batch_size, weight_path, device, transform, version=50, num_classes=100, img_pixels= (224, 224)):
    ...
    train_set_X, train_set_y, _,_ = data_utils.process_data(train_path)
...
def main(train_path, batch_size, weight_path, num_classes, glod_k = 100, quantile=0.05):
    ...
    # Load data
    in_set_X, in_set_y, _ ,in_race= data_utils.process_data(in_path)
    out_set_X, out_set_y, _,out_race = data_utils.process_data(out_path)
    ...
    # Load original model
    model = create_Resnet(resnet_number=50, num_classes=num_class,
                      gaussian_layer=False, weight_path=weight_path,
                      device=device)

    # Convert to Glod
    glod_model = Convert(train_path, batch_size, weight_path, device)
    ...
    visualize_distribution(in_scores, out_scores, model_type, data_type, glod_k)
